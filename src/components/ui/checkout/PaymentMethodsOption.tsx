"use client";

import Image from "next/image";
import React, { useState } from "react";
import code from "@/assets/checkout/code.svg";
import bkash from "@/assets/checkout/bkash.svg";
import nogot from "@/assets/checkout/nogot.svg";
import rocket from "@/assets/checkout/roket.svg";

type PaymentMethodsOptionProps = {};

const PaymentMethodsOption: React.FC<PaymentMethodsOptionProps> = () => {
  const [selectedMethod, setSelectedMethod] = useState(1);

  const paymentMethods = [
    { id: 1, name: "Cash on delivery", image: code },
    { id: 2, name: "Pay with Bkash", image: bkash },
    { id: 3, name: "Pay with Nagad", image: nogot },
    { id: 4, name: "Pay with Rocket", image: rocket },
  ];
  return (
    <div className="">
      <div className="grid grid-cols-1 gap-4 mt-5">
        {paymentMethods.map((method) => (
          <div key={method.id} className="flex items-center justify-between">
            <label className={` rounded-lg cursor-pointer`}>
              <input
                type="radio"
                name="paymentMethod"
                value={method.id}
                checked={selectedMethod === method.id}
                onChange={() => setSelectedMethod(method.id)}
                className="hidden"
              />

              <div className="flex items-center gap-[14px]">
                {selectedMethod === method.id ? (
                  <div className=" w-[18px]">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="18"
                      height="18"
                      viewBox="0 0 18 18"
                      fill="none"
                    >
                      <g clip-path="url(#clip0_1834_26120)">
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M9 0.75C4.44375 0.75 0.75 4.44375 0.75 9C0.75 13.5562 4.44375 17.25 9 17.25C13.5562 17.25 17.25 13.5562 17.25 9C17.25 4.44375 13.5562 0.75 9 0.75ZM12.576 7.605C12.6418 7.52973 12.692 7.44205 12.7234 7.34713C12.7549 7.2522 12.7671 7.15194 12.7592 7.05224C12.7513 6.95254 12.7236 6.85542 12.6777 6.7666C12.6317 6.67777 12.5685 6.59903 12.4917 6.53501C12.4148 6.47098 12.326 6.42297 12.2303 6.3938C12.1347 6.36462 12.0341 6.35487 11.9347 6.36512C11.8352 6.37537 11.7388 6.40541 11.6511 6.45347C11.5634 6.50154 11.4862 6.56666 11.424 6.645L8.199 10.5142L6.53025 8.84475C6.3888 8.70813 6.19935 8.63254 6.0027 8.63424C5.80605 8.63595 5.61794 8.71483 5.47889 8.85389C5.33983 8.99294 5.26095 9.18105 5.25924 9.3777C5.25754 9.57435 5.33313 9.7638 5.46975 9.90525L7.71975 12.1553C7.79344 12.2289 7.88167 12.2864 7.97881 12.324C8.07595 12.3617 8.17987 12.3787 8.28395 12.374C8.38803 12.3693 8.48998 12.3429 8.58331 12.2966C8.67663 12.2503 8.75929 12.185 8.826 12.105L12.576 7.605Z"
                          fill="#0198E9"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_1834_26120">
                          <rect width="18" height="18" fill="white" />
                        </clipPath>
                      </defs>
                    </svg>
                  </div>
                ) : (
                  <div className=" w-[18px]">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
                      viewBox="0 0 16 16"
                      fill="none"
                    >
                      <circle
                        cx="8"
                        cy="8"
                        r="7.125"
                        stroke="#999999"
                        stroke-opacity="0.6"
                        stroke-width="1.75"
                      />
                    </svg>
                  </div>
                )}
                <span className="text-[#757575] tracking-[0.32px]">
                  {" "}
                  {method.name}
                </span>
              </div>
            </label>
            <div>
              <Image src={method.image} alt={method.image} />
            </div>
          </div>
        ))}
      </div>
    </div>
  );
};
export default PaymentMethodsOption;
